...
const mockVoices = [
  { id: '1', name: '温柔女声 (参考音频A)' },
  { id: '2', name: '沉稳男声 (参考音频B)' },
  { id: '3', name: '活力男声 (参考音频C)' },
];

const handleSynthesize = async () => {
  if (!text.value || isTextOverLimit.value) return;
  isSynthesizing.value = true;
  
  // 模拟 API 调用延迟
  setTimeout(() => {
    isSynthesizing.value = false;
    audioUrl.value = 'dummy_url';
  }, 2000);
};
    ...
    <div class="container">
      <div class="left-panel">
        <div class="card editor-card">
          ...
          ></textarea>
          <div class="toolbar">
            <BaseButton type="text" size="sm" @click="clearText">
              <Trash2 :size="14" /> 清空文本
            </BaseButton>
            <BaseButton 
              type="primary" 
        ...
        <div class="card settings-card">
          ...
          <div class="field">
            ...
            <select v-model="voiceId" class="select">
              <option value="">默认声音 (Generic)</option>
              <option v-for="v in mockVoices" :key="v.id" :value="v.id">{{ v.name }}</option>
            </select>
          </div>
          <div class="field">
            <div class="label-row">
      ...
      <div class="right-panel">
        <div class="card player-card">
          ...
          <div v-if="isSynthesizing" class="skeleton">
            ...
            <p>正在执行 In-context Learning...</p>
          </div>

          <div v-if="audioUrl && !isSynthesizing" class="player">
            <div class="waveform">
              <!-- 这里以后可以集成 Wavesurfer.js -->
              <div class="wave-bar"></div>
            </div>
            <div class="player-actions">
              <BaseButton type="primary" size="lg" round>
                <Play fill="white" :size="20" />
              </BaseButton>
              <BaseButton type="secondary" size="md">
...
.waveform { height: 80px; width: 100%; background: var(--color-primary-light); border-radius: 8px; margin-bottom: 20px; }
.player-actions { display: flex; gap: 12px; justify-content: center; align-items: center; }

.history-card { min-height: 200px; }
.empty { font-size: 13px; color: var(--color-text-disabled); text-align: center; padding: 40px 0; }
</style>
